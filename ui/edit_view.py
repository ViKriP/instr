...
from apps.instructions.services import (
    add_dependency,
    add_task_with_solution,
    create_instruction,
    delete_dependency,
    delete_instruction,
    delete_task,
...
def InstructionEditView(page: ft.Page, inst_id=None):
    ...
    def save_task_click(e):
        if task_name.value and task_cmd.value:
            ...
            if editing_task_id:
                update_task_with_solution(
                    editing_task_id, task_name.value, seq, task_cmd.value
                )
            else:
    ...
    task_dialog = ft.AlertDialog(
        content=ft.Column(
            [ft.Row([task_seq, task_name]), task_cmd], tight=True, width=500
        ),
        actions=[
    ...
    def open_task_edit(e, task_data=None):
        ...
        if task_data:
            ...
            cmd = (
                task_data["solutions"][0]["exec_command"]
                if task_data["solutions"]
                else ""
            )
            task_cmd.value = cmd
    ...
    def execute_child_delete(e):
        if pending_delete_type == "dep":
            delete_dependency(pending_delete_id)
        else:
    ...
    def req_main_del(e):
        ...
        main_del_text.value = (
            f"Вы уверены, что хотите удалить '{data['name']}' со всеми задачами?"
        )
    ...
    def load_data():
        ...
        dependencies_list.controls = [
            ft.ListTile(
                ...
                subtitle=ft.Text(
                    value=d["check_command"], size=12, font_family="monospace"
                ),
                trailing=ft.Row(
                    [
                        ft.IconButton(
                            ft.Icons.EDIT,
                            on_click=lambda _, d=d: open_dep_edit(None, d),
                        ),
                        ft.IconButton(
                            ft.Icons.DELETE,
                            on_click=lambda _, d=d: req_child_del(
                                d["id"], d["name"], "dep"
                            ),
                        ),
                    ],
        ...
        tasks_list.controls = [
            ft.ListTile(
                ...
                subtitle=ft.Text(
                    value=t["solutions"][0]["exec_command"]
                    if t["solutions"]
                    else "No command",
                    max_lines=1,
                ...
                trailing=ft.Row(
                    [
                        ft.IconButton(
                            ft.Icons.EDIT,
                            on_click=lambda _, t=t: open_task_edit(None, t),
                        ),
                        ft.IconButton(
                            ft.Icons.DELETE,
                            on_click=lambda _, t=t: req_child_del(
                                t["id"], t["name"], "task"
                            ),
                        ),
                    ],
    ...
    if is_editing:
        app_bar_actions.insert(
            1,
            ft.IconButton(
                ft.Icons.DELETE_FOREVER, icon_color=ft.Colors.RED, on_click=req_main_del
            ),
        )

    return ft.View(
        route=f"/edit/{inst_id}" if inst_id else "/create",
        controls=[
                ...
                [
                    ...
                    ft.Row(
                        [
                            ...
                            ft.IconButton(
                                ft.Icons.ADD_CIRCLE,
                                on_click=lambda e: open_dep_edit(e, None),
                    ...
                    ft.Row(
                        [
                            ft.Text("Задачи", size=20, weight="bold"),
                            ft.IconButton(
                                ft.Icons.ADD_CIRCLE,
                                on_click=lambda e: open_task_edit(e, None),
