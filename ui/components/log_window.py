...
def build_log_dialog(page: ft.Page):
    ...
    Здесь находится вся настройка вида и кнопок.
    """
    ...
    async def copy_logs(e):
        # Собираем текст
        full_text = "\n".join(
            [c.value for c in log_view.controls if isinstance(c, ft.Text)]
        )
        if full_text:
            try:
                ...
                page.show_dialog(
                    ft.SnackBar(ft.Text("Скопировано!"), bgcolor=ft.Colors.GREEN_700)
                )
